...
{
    public class UpdateDealValidatorBL : IValidator<AcceptUpdateDealDtoBL>
    {
        public readonly IServiceAccountingContext context;

        public UpdateDealValidatorBL(IServiceAccountingContext context)
        {
            this.context = context;
        }

        public async Task Validate(AcceptUpdateDealDtoBL dto)
        {
            if (dto is null)
                throw new ElementNullReferenceException($"{nameof(AcceptCreateDealDtoBL)} is null");

            if (await Task.Factory.StartNew(() => !context.Set<Deal>().AsNoTracking().ToList().Exists(x => x.Id == dto.Id)))
