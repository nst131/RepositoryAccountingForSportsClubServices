    ...
    public class UpdateClubCardValidatorBL : IValidator<AcceptUpdateClubCardDtoBL>
    {
        ...
        public async Task Validate(AcceptUpdateClubCardDtoBL updateClubCardDtoBL)
        {
            ...
            if (string.IsNullOrWhiteSpace(updateClubCardDtoBL.Name))
                throw new ElementNotAssignException($"{nameof(AcceptUpdateClubCardDtoBL.Name)} is not assigned");

            if (updateClubCardDtoBL.Price < 0)
                throw new ElementOutOfRangeException($"{nameof(AcceptUpdateClubCardDtoBL.Price)} can not less 0");

            if (updateClubCardDtoBL.DurationInDays < 0)
                throw new ElementOutOfRangeException($"{nameof(AcceptUpdateClubCardDtoBL.DurationInDays)} can not less 0");
